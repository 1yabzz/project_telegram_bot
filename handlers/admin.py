...
async def g_admin(message: types.Message,  state:FSMContext):
    """функция принимает в себя сообщение, проверяет айди человека со списком админов,
    если совпадает, сохраняет в себя отправленный ранее айди, завершает работу машины состояний
    и отправляет сообщение"""
    if message.from_user.id == MAIN_ADMIN:
        async with state.proxy() as data:
...
async def d_admin(message:types.Message, state:FSMContext):
    """функция принимает в себя сообщение, проверяет айди человека со списком админов,
    если совпадает, сохраняет в себя отправленный ранее айди, удалет из списка,
    завершает работу машины состояний
    и отправляет сообщение"""
    if message.from_user.id == MAIN_ADMIN:
        async with state.proxy() as data:
            data['admin'] = message.text
